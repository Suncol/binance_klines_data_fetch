"""Background-updating Binance Options partial depth service."""
...
from .options import (
    DEFAULT_OPTIONS_WS_BASE_URL,
    MAX_OPTIONS_STREAMS_PER_CONNECTION,
    OPTIONS_PUBLIC_WS_ENDPOINT,
    VALID_OPTIONS_DEPTH_LEVELS,
    VALID_OPTIONS_DEPTH_SPEEDS_MS,
    OptionDepthLevel,
    OptionDepthSpeedMs,
    OptionSymbol,
)
...
class OptionsDepthSnapshot:
    ...
    bids: tuple[OptionDepthPriceLevel, ...]
    asks: tuple[OptionDepthPriceLevel, ...]
    mid_price: Optional[Decimal]
    spread: Optional[Decimal]
    spread_bps: Optional[Decimal]
    expected_levels: int
    depth_incomplete: bool
    is_stale: bool
    sequence_gap: bool
    ...
    def get_level_quote(self, level: int = 1) -> DepthLevelQuote:
        ...
        return DepthLevelQuote(
            ...
            depth_incomplete=self.depth_incomplete,
        )


@dataclass(frozen=True)
class BinanceOptionsDepthConfig:
    symbols: Iterable[str | OptionSymbol]
    ...
    def __post_init__(self) -> None:
        symbols = _normalize_option_symbols(self.symbols)
        if not symbols:
            raise ValueError("symbols must contain at least one symbol")
        if len(symbols) > MAX_OPTIONS_STREAMS_PER_CONNECTION:
            raise ValueError(f"symbols must contain at most {MAX_OPTIONS_STREAMS_PER_CONNECTION} symbols")
        if self.levels not in VALID_OPTIONS_DEPTH_LEVELS:
            raise ValueError(f"levels must be one of {sorted(VALID_OPTIONS_DEPTH_LEVELS)}, got {self.levels}")
        if self.speed_ms not in VALID_OPTIONS_DEPTH_SPEEDS_MS:
            raise ValueError(f"speed_ms must be one of {sorted(VALID_OPTIONS_DEPTH_SPEEDS_MS)}, got {self.speed_ms}")
...
@dataclass(frozen=True)
class OptionsDepthServiceStatus:
    symbols: tuple[str, ...]
    running: bool
    ready: bool
    connected: bool
...
class BinanceOptionsDepthService:
    """Maintain latest top-N Binance Options bid and ask levels in a background thread."""
    ...
    def __init__(
        ...
        *,
        ...
        self._snapshots: dict[str, OptionsDepthSnapshot] = {}
    ...
    def start(self, *, block_until_ready: bool = False, timeout: Optional[float] = None) -> None:
        ...
        self._thread = threading.Thread(
            ...
            name="binance-options-depth-service",
            daemon=True,
        )
        self._thread.start()
    ...
    def get_latest(self, symbol: str | OptionSymbol) -> Optional[OptionsDepthSnapshot]:
        symbol = _normalize_option_symbol(symbol)
        with self._lock:
            return self._snapshots.get(symbol)

    def get_latest_snapshot(self, symbol: str | OptionSymbol) -> Optional[OptionsDepthSnapshot]:
        return self.get_latest(symbol)

    def get_latest_level_quote(
        self,
        symbol: str | OptionSymbol,
        level: int = 1,
        *,
    ...
    @property
    def latest_depth_table(self) -> dict[str, OptionsDepthSnapshot]:
        return self.get_all_latest()

    def status(self) -> OptionsDepthServiceStatus:
        with self._lock:
            return OptionsDepthServiceStatus(
    ...
    def __enter__(self) -> "BinanceOptionsDepthService":
    ...
    def _stream_name(self, symbol: str) -> str:
        return f"{symbol.lower()}@depth{self.config.levels}@{self.config.speed_ms}ms"

    @staticmethod
    def _snapshot_passes_level_filters(
        snapshot: OptionsDepthSnapshot,
        *,
    ...
    def _wait_until_ready(self, timeout: Optional[float]) -> None:
        ...
        raise DepthServiceError(f"options depth service did not become ready within {wait_timeout}s{detail}")
    ...
    async def _run_forever(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            try:
                self.logger.info("Connecting to Binance Options depth stream: url=%s", url)
                ...
                self.logger.warning(
                    "Options WebSocket connection closed: code=%s reason=%s",
                ...
                self.logger.warning("Options WebSocket network-level error: %r", exc)
                self._record_error(exc)
                self._mark_all_stale("network_error")
            except Exception as exc:
                self.logger.exception("Unexpected Options WebSocket error: %r", exc)
                self._record_error(exc)
            ...
            self.logger.warning(
                "Reconnecting Binance Options depth stream after %.2f seconds",
                sleep_seconds,
            )
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if self._read_timeout_elapsed(last_recv_monotonic):
                    self.logger.warning(
                        "No Options WebSocket message received for %.1f seconds; reconnecting",
    ...
    def _handle_raw_message(self, raw: str | bytes) -> None:
        try:
            ...
            self.logger.warning("Failed to parse Options depth message: error=%r raw_prefix=%s", exc, _raw_prefix(raw))
            self._record_error(exc)
            return
    ...
    def _parse_depth_message(self, raw: str | bytes | Mapping[str, Any]) -> OptionsDepthSnapshot:
        ...
        if isinstance(raw, str):
            try:
                ...
                raise BinanceDepthResponseError(f"invalid JSON from Options depth stream: {exc}") from exc
        elif isinstance(raw, Mapping):
            message = raw
        else:
            raise BinanceDepthResponseError("Options depth message must be a JSON string, bytes, or mapping")

        if not isinstance(message, Mapping):
            raise BinanceDepthResponseError("Options depth message must decode to a JSON object")
        data = message.get("data", message)
        if not isinstance(data, Mapping):
            raise BinanceDepthResponseError("Options depth message data must be a JSON object")

        missing = [key for key in ("s", "E", "T", "U", "u", "pu", "b", "a") if key not in data]
        if missing:
            raise BinanceDepthResponseError(f"Options depth message missing required fields: {missing}")

        local_recv_time_ms = utc_now_ms()
        symbol = _normalize_option_symbol(str(data["s"]))
        ...
        return OptionsDepthSnapshot(
            ...
            expected_levels=int(self.config.levels),
            depth_incomplete=len(bids) < self.config.levels or len(asks) < self.config.levels,
            is_stale=False,
            sequence_gap=False,
        )

    def _with_sequence_status_locked(self, snapshot: OptionsDepthSnapshot) -> OptionsDepthSnapshot:
        ...
        if sequence_gap:
            self.logger.warning(
                "Options depth sequence gap detected: symbol=%s previous_u=%s current_pu=%s current_u=%s",
                snapshot.symbol,
                previous_u,
...
def build_options_depth_service_configs(
    symbols: Iterable[str | OptionSymbol],
    *,
    levels: OptionDepthLevel = 5,
    speed_ms: OptionDepthSpeedMs = 100,
    ...
    startup_timeout_seconds: float = 30.0,
    max_queue: int = 8192,
) -> list[BinanceOptionsDepthConfig]:
    if (
    ...
    normalized_symbols = _normalize_option_symbols(symbols)
    ...
    return [
        BinanceOptionsDepthConfig(
            ...
            reconnect_jitter_seconds=reconnect_jitter_seconds,
            startup_timeout_seconds=startup_timeout_seconds,
            max_queue=max_queue,
        )
        for group in _chunked(list(normalized_symbols), max_streams_per_connection)
    ]


def _normalize_option_symbol(symbol: str | OptionSymbol) -> str:
    raw_symbol = symbol.symbol if isinstance(symbol, OptionSymbol) else symbol
    if not isinstance(raw_symbol, str) or not raw_symbol.strip():
        raise ValueError("symbol must be a non-empty string")
    return raw_symbol.strip().upper()


def _normalize_option_symbols(symbols: Iterable[str | OptionSymbol]) -> tuple[str, ...]:
    if isinstance(symbols, (str, OptionSymbol)):
        symbols = [symbols]
...
def _parse_price_qty_array(
    ...
    *,
    ...
    max_levels: int,
) -> tuple[OptionDepthPriceLevel, ...]:
    if not isinstance(values, list):
        raise BinanceDepthResponseError(f"Options {side} levels must be a JSON array")

    parsed: list[tuple[Decimal, Decimal]] = []
    for index, row in enumerate(values):
        if not isinstance(row, (list, tuple)) or len(row) < 2:
            raise BinanceDepthResponseError(f"malformed Options {side} level at index {index}")
        try:
            price = Decimal(str(row[0]))
            qty = Decimal(str(row[1]))
        except Exception as exc:
            raise BinanceDepthResponseError(f"invalid Options {side} price/qty at index {index}") from exc
        parsed.append((price, qty))

    parsed.sort(key=lambda item: item[0], reverse=side == "bid")
    return tuple(
        OptionDepthPriceLevel(level=level, price=price, qty=qty)
...
def _coerce_int(value: Any, field_name: str) -> int:
    try:
        ...
        raise BinanceDepthResponseError(f"Options depth field {field_name} must be an integer") from exc


def _chunked(items: list[str], chunk_size: int) -> list[tuple[str, ...]]:
